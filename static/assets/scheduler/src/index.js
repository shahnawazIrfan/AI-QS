(function ($) {
  ...
  DayScheduleSelector.prototype.renderHeader = function () {
    ...
    $.each(days, function (i, _) { html += '<th style="color:#193a83;text-align:center;background-color:white">' + (stringDays[i] || '') + '<br>' + (Date[i] || '') + '</th>'; });
      ...
      if (!plugin.isSelecting()) {  // if we are not in selecting mode
        ...
        if (day == plugin.$selectingStart.data('day')) {  // if clicking on the same day column
          ...
          plugin.$el.find('.time-slot[data-day="' + day + '"]').filter('[data-selecting]')
            .attr('data-reserved', 'reserved').removeAttr('data-selecting');
          ...
          $(this).css('background-color','#193a83');
          $(this).css('color','white');
          $('.schedule-rows .time-slot').not(this).removeAttr('data-reserved');
          $('.schedule-rows .time-slot').not(this).removeAttr('style');
        }
      }
    });
    ...
    $.each(schedule, function(d, ds) {
      ...
      $.each(ds, function(_, s) {
        ...
        for (i = 0; i < $slots.length; i++) {
          ...
          if ($slots.eq(i).data('time') == s[0] || $slots.eq(i).data('time') == s[1] || $slots.eq(i).data('time') == s[2] || $slots.eq(i).data('time') == s[3] || $slots.eq(i).data('time') == s[4] || $slots.eq(i).data('time') == s[5] || $slots.eq(i).data('time') == s[6] || $slots.eq(i).data('time') == s[7] || $slots.eq(i).data('time') == s[8] || $slots.eq(i).data('time') == s[9] || $slots.eq(i).data('time') == s[10] || $slots.eq(i).data('time') == s[11] || $slots.eq(i).data('time') == s[12] || $slots.eq(i).data('time') == s[13] || $slots.eq(i).data('time') == s[14] || $slots.eq(i).data('time') == s[15] || $slots.eq(i).data('time') == s[16] || $slots.eq(i).data('time') == s[17] || $slots.eq(i).data('time') == s[18] || $slots.eq(i).data('time') == s[19] || $slots.eq(i).data('time') == s[20] || $slots.eq(i).data('time') == s[21] || $slots.eq(i).data('time') == s[22] || $slots.eq(i).data('time') == s[23]) 
